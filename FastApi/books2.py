...
class BookRequest(BaseModel):
    ...
    published_date: int = Field(gt=1700, lt=2027)
...
async def read_book_by_published_date(published_date: int = Query(gt=1700, lt=2027)):
    Books_to_return = []
    for book in BOOKS:
...
async def fetch_book_by_book_id(book_id: int = Path(gt=0)):
    for book in BOOKS:
        if book.id == book_id:
            return book
